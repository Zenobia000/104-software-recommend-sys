...
def index():
    ...
    if current_user.is_active:
        return render_template('index.html', uname = current_user.id, df = datalist)
    else:
        return render_template('index.html', df = datalist)
...
def search():
    if request != '':
        ...
        print(joblist)
    if current_user.is_active:
        return render_template('job-search.html', uname = current_user.id, joblist = joblist, df = datalist)
    return render_template('job-search.html', joblist = joblist, df = datalist)

@app.route('/job-detail', methods=['GET','POST'])
def jobdetail():
    if current_user.is_active:
        return render_template('job-detail.html',uname = current_user.id)
